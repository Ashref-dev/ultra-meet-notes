...
Injects CSV-based transcript data into the Ultra Meet SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "tn.ashref.ultrameet"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "tn.ashref.ultrameet"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "tn.ashref.ultrameet"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "tn.ashref.ultrameet"
...
def process_segments(segments: list[dict[str, str]], start_time: datetime) -> list[dict[str, Any]]:
    """
...
def verify_injection(db_path: str, meeting_id: str) -> dict[str, Any]:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Ultra Meet database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure Ultra Meet has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Ultra Meet sidebar.")
