...
function loadBetaFeatures(): BetaFeatures {
...
function saveBetaFeatures(features: BetaFeatures): void {
  if (typeof window === 'undefined') {
    return;
  }
...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  const [transcriptModelConfig, setTranscriptModelConfig] = useState<TranscriptModelProps>({
    ...
    model: getDefaultTranscriptionModel(DEFAULT_TRANSCRIPT_PROVIDER),
    apiKey: null
  });
  ...
  const loadPreferences = useCallback(async () => {
    ...
    try {
      ...
      const [dbDir, modelsDir, saveLocations] = await Promise.all([
        invoke<string>('get_database_directory'),
        invoke<string>('whisper_get_models_directory'),
        configService.getSaveLocations()
      ]);

      setStorageLocations({
        database: dbDir,
        models: modelsDir,
        recordings: saveLocations.recordings_folder,
        notes: saveLocations.notes_folder,
        syncFolders: saveLocations.sync_folders,
      });
  ...
  useEffect(() => {
    const handleRecordingLocationUpdated = (event: Event) => {
      const customEvent = event as CustomEvent<{
        recordingsPath?: string;
        notesPath?: string;
        syncFolders?: boolean;
      }>;
      const nextRecordingsPath = customEvent.detail?.recordingsPath;
      const nextNotesPath = customEvent.detail?.notesPath;
      const nextSyncFolders = customEvent.detail?.syncFolders;

      if (!nextRecordingsPath && !nextNotesPath && nextSyncFolders === undefined) {
        return;
      }
      ...
      setStorageLocations((current) => {
        ...
        return {
          ...current,
          recordings: nextRecordingsPath ?? current.recordings,
          notes: nextNotesPath ?? current.notes,
          syncFolders: nextSyncFolders ?? current.syncFolders,
        };
      });
    };

    window.addEventListener(RECORDING_LOCATION_UPDATED_EVENT, handleRecordingLocationUpdated as EventListener);

    return () => {
      window.removeEventListener(RECORDING_LOCATION_UPDATED_EVENT, handleRecordingLocationUpdated as EventListener);
    };
  }, []);
