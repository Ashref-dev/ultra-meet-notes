...
export type OnboardingTranscriptionProvider = 'parakeet' | 'whisper';
...
interface OnboardingContextType {
  currentStep: number;
  selectedTranscriptionProvider: OnboardingTranscriptionProvider;
  includeSummaryModel: boolean;
  includeTranscriptionModel: boolean;
  ...
  goPrevious: () => void;
  // Setters
  setSelectedTranscriptionProvider: (value: OnboardingTranscriptionProvider) => void;
  setIncludeSummaryModel: (value: boolean) => void;
  setIncludeTranscriptionModel: (value: boolean) => void;
...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const [selectedTranscriptionProvider, setSelectedTranscriptionProvider] =
    useState<OnboardingTranscriptionProvider>('parakeet');
  const [includeSummaryModel, setIncludeSummaryModel] = useState(false);
  const [includeTranscriptionModel, setIncludeTranscriptionModel] = useState(false);
  ...
  const verifyModelStatus = useCallback(async (savedStatus: OnboardingStatus) => {
    let parakeetDownloaded = false;
    let summaryModelDownloaded = false;

    // Verify selected transcription model exists on disk. Treat the generic
    // transcription download flag as satisfied when either supported provider is available.
    try {
      await invoke('parakeet_init');
      const hasParakeet = await invoke<boolean>('parakeet_has_available_models');

      await invoke('whisper_init');
      const hasWhisper = await invoke<boolean>('whisper_has_available_models');

      parakeetDownloaded = hasParakeet || hasWhisper;
      console.log('[OnboardingContext] Transcription models verified on disk:', {
        hasParakeet,
        hasWhisper,
      });
    } catch (error) {
      console.warn('[OnboardingContext] Failed to verify transcription models:', error);
    ...
    // New simplified flow: Step 1: Welcome, Step 2: Setup Overview, Step 3: Model Choice,
    // Step 4: Download Progress, Step 5: Permissions (macOS)
  ...
  }, [checkActiveDownloads, verifyModelStatus]);
  ...
  useEffect(() => {
    ...
    fetchRecommendation();
  }, [checkDatabaseStatus, initializeDatabaseInBackground, loadOnboardingStatus]);
  ...
  }, [completed, saveOnboardingStatus]);
  ...
  // Start background downloads for the selected models.
  const startBackgroundDownloads = async () => {
    ...
    try {
      if (includeTranscriptionModel && !parakeetDownloaded) {
        if (selectedTranscriptionProvider === 'parakeet') {
