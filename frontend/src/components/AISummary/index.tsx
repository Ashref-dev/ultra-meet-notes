...
const generateUniqueId = (sectionKey: string) => `${sectionKey}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  useEffect(() => {
    ...
    if (!isUndoRedoing) {
      setHistory((previousHistory) => {
        const newHistory = previousHistory.slice(0, historyIndexRef.current + 1);
    ...
    <div className="relative">
      ...
      {/* <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-2xl font-semibold bg-gradient-to-r from-accent to-accent/70 bg-clip-text text-transparent">
            AI Enhanced Summary
          </h2>
        </div>
        <div className="flex items-center space-x-2">
