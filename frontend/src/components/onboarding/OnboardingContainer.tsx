    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-background">
      <div className={cn('w-full max-w-2xl h-full max-h-screen flex flex-col px-6 py-6', className)}>
            ...
            {showNavigation && (
              <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between pointer-events-none">
                <button
                  ...
                  className={cn(
                    'pointer-events-auto flex h-8 w-8 items-center justify-center rounded-full border border-border bg-card shadow-sm transition-all duration-200',
                    canGoPrevious && step !== 1
                      ? 'text-foreground hover:scale-110 hover:bg-muted hover:shadow-md'
        ...
        <div className="mb-4 text-center space-y-3 flex-shrink-0">
          <h1 className="animate-fade-in-up text-4xl font-semibold text-foreground">{title}</h1>
          {description && (
            <p className="mx-auto max-w-md animate-fade-in-up text-base text-muted-foreground delay-75">
              {description}
            </p>
          )}
        </div>
