...
export function PermissionRow({ icon, title, description, status, isPending = false, onAction }: PermissionRowProps) {
  ...
  return (
    <div
      className={cn(
        'flex items-center justify-between rounded-xl border px-6 py-5',
        'transition-all duration-200',
        isAuthorized ? 'border-border bg-muted' : isDenied ? 'border-destructive/30 bg-destructive/10' : 'border-border bg-card'
      )}
    >
      {/* Left side: Icon + Info */}
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div
          className={cn(
            ...
            isAuthorized ? 'bg-background' : isDenied ? 'bg-destructive/10' : 'bg-muted'
          )}
        >
