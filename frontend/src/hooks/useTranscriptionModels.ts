...
export function useTranscriptionModels(transcriptModelConfig: TranscriptModelConfig | undefined) {
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      const availableWhisper = whisperModels
        ...
        .map((m) => ({
          ...
          name: m.name,
          displayName: `Local Whisper: ${m.name}`,
          size_mb: m.size_mb,
        }));
    ...
    const configuredProvider = transcriptModelConfig?.provider || DEFAULT_TRANSCRIPT_PROVIDER;
