...
const TRAY_ICON_RECORDING_PNG: &[u8] = include_bytes!("../icons/tray-icon-recording@2x.png");

fn get_tray_icon() -> tauri::image::Image<'static> {
    tauri::image::Image::from_bytes(TRAY_ICON_PNG)
        .expect("embedded tray-icon@2x.png must be a valid PNG")
}

fn get_tray_icon_recording() -> tauri::image::Image<'static> {
    tauri::image::Image::from_bytes(TRAY_ICON_RECORDING_PNG)
        .expect("embedded tray-icon-recording@2x.png must be a valid PNG")
}
...
pub fn update_tray_icon<R: Runtime>(app: &AppHandle<R>, is_recording: bool) {
    if let Some(tray) = app.tray_by_id("main-tray") {
        if is_recording {
            let _ = tray.set_icon(Some(get_tray_icon_recording()));
            let _ = tray.set_icon_as_template(false);
        } else {
            let _ = tray.set_icon(Some(get_tray_icon()));
