...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    let (preferences, _user_wants_keep_audio, preferred_mic_name, preferred_system_name) =
        match super::recording_preferences::load_recording_preferences(&app).await {
            Ok(prefs) => {
                info!("📋 Loaded recording preferences: keep_audio_after_analysis={}, preferred_mic={:?}, preferred_system={:?}",
    ...
    manager.set_recording_preferences(preferences);
...
pub async fn stop_recording<R: Runtime>(
    ...
    let (meeting_folder, notes_folder, meeting_name) = if let Some(mut manager) = manager_for_cleanup {
        info!("🧹 Performing final cleanup and saving recording data");

        // Extract meeting info BEFORE async operations
        let meeting_folder = manager.get_meeting_folder();
        let notes_folder = manager.get_notes_folder();
        ...
        (meeting_folder, notes_folder, meeting_name)
    } else {
    ...
    let (folder_path_str, notes_folder_path_str, meeting_name_str) = match (&meeting_folder, &notes_folder, &meeting_name) {
        (Some(path), Some(notes_path), Some(name)) => (
            Some(path.to_string_lossy().to_string()),
            Some(notes_path.to_string_lossy().to_string()),
            Some(name.clone()),
        ),
        _ => (None, None, None),
    };
    ...
    info!("   notes_folder_path: {:?}", notes_folder_path_str);
