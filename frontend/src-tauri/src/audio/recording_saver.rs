...
impl RecordingSaver {
    ...
    pub fn set_preferences(&mut self, preferences: RecordingPreferences) {
    ...
    pub fn set_device_info(&mut self, mic_name: Option<String>, sys_name: Option<String>) {
        if let Some(ref mut metadata) = self.metadata {
            ...
            if let Some(folder) = &self.notes_folder {
    ...
    fn initialize_meeting_folder(&mut self, meeting_name: &str, create_checkpoints: bool) -> Result<()> {
        ...
        let notes_base_folder = if preferences.sync_folders {
            preferences.save_folder.clone()
        } else {
            preferences.notes_folder.clone()
        };

        let recordings_folder = create_meeting_folder(&preferences.save_folder, meeting_name, create_checkpoints)?;
        let notes_folder = if notes_base_folder == preferences.save_folder {
            recordings_folder.clone()
        } else {
            let folder_name = recordings_folder
                .file_name()
                .map(|name| name.to_os_string())
                .unwrap_or_else(|| {
                    let timestamp = chrono::Utc::now().format("%Y-%m-%d_%H-%M").to_string();
                    format!("{}_{}", sanitize_filename(meeting_name), timestamp).into()
                });
            let notes_folder = notes_base_folder.join(folder_name);
        ...
        if create_checkpoints {
            let incremental_saver = IncrementalAudioSaver::new(recordings_folder.clone(), 48000)?;
        ...
        let metadata = MeetingMetadata {
            ...
            audio_file: if create_checkpoints {
                if notes_folder == recordings_folder {
                    "audio.mp4".to_string()
                } else {
                    recordings_folder.join("audio.mp4").to_string_lossy().to_string()
                }
            } else {
                "".to_string()
            },
        ...
        self.write_metadata(&notes_folder, &metadata)?;

        self.recordings_folder = Some(recordings_folder);
        self.notes_folder = Some(notes_folder);
    ...
    pub async fn stop_and_save<R: Runtime>(
        ...
        if let (Some(folder), Some(mut metadata)) = (&self.notes_folder, self.metadata.clone()) {
        ...
        let save_event = serde_json::json!({
            ...
            "meeting_folder": self.recordings_folder.as_ref()
    ...
    pub fn get_meeting_folder(&self) -> Option<&PathBuf> {
        self.recordings_folder.as_ref()
    }

    pub fn get_notes_folder(&self) -> Option<&PathBuf> {
        self.notes_folder.as_ref()
    }
