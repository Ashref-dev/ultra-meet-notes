...
impl RecordingManager {
    ...
    pub fn set_recording_preferences(&mut self, preferences: RecordingPreferences) {
        self.recording_saver.set_preferences(preferences);
    }
    ...
    pub fn get_notes_folder(&self) -> Option<std::path::PathBuf> {
        self.recording_saver.get_notes_folder().map(|p| p.clone())
    }
