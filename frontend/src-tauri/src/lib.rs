...
#[tauri::command]
fn toggle_ghost_mode<R: Runtime>(app: AppHandle<R>) -> bool {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .manage(meeting_app_detector::MeetingAppDetectorState::default())
        ...
        .setup(|_app| {
            ...
            // The dictation feature isn't surfaced in Ultra Meet's UI and the listener
            // requires Accessibility + Input Monitoring permissions, which prompted
            // users on every launch. The dictation Rust module + commands are kept
            // in the codebase as dead code in case the feature is ever re-enabled.
            // Re-enable by calling dictation::start_global_hotkey_listener(&_app.handle()).
            ...
            let detector_app_handle = _app.handle().clone();
            tauri::async_runtime::spawn(async move {
                match audio::recording_preferences::load_recording_preferences(&detector_app_handle).await {
                    Ok(preferences) => {
                        if preferences.meeting_app_detection_enabled {
                            let detector_state = detector_app_handle.state::<meeting_app_detector::MeetingAppDetectorState>();
                            if let Err(error) = detector_state.start(detector_app_handle.clone()) {
                                log::error!("Failed to start meeting app detector: {}", error);
                            }
                        } else {
                            log::info!("Meeting app detector disabled by saved preference");
                        }
                    }
                    Err(error) => {
                        log::error!("Failed to load recording preferences for meeting app detector: {}", error);
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            audio::recording_preferences::get_save_locations,
            audio::recording_preferences::set_recordings_folder,
            audio::recording_preferences::set_notes_folder,
            audio::recording_preferences::set_sync_folders,
            audio::recording_preferences::get_default_recordings_folder_path,
            audio::recording_preferences::open_recordings_folder,
            audio::recording_preferences::open_notes_folder,
            audio::recording_preferences::select_recording_folder,
            audio::recording_preferences::select_notes_folder,
            audio::recording_preferences::get_available_audio_backends,
            audio::recording_preferences::get_current_audio_backend,
            audio::recording_preferences::set_audio_backend,
            audio::recording_preferences::get_audio_backend_info,
            meeting_app_detector::enable_meeting_app_detection,
