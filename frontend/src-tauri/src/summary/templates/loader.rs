...
/// - macOS: ~/Library/Application Support/tn.ashref.ultrameet/templates/
/// - Windows: %APPDATA%\tn.ashref.ultrameet\templates\
/// - Linux: ~/.config/tn.ashref.ultrameet/templates/
pub(crate) fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push("tn.ashref.ultrameet");
    path.push("templates");
    Some(path)
}
