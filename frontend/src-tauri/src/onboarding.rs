...
pub async fn complete_onboarding<R: Runtime>(
    ...
    info!(
        "Completing onboarding with builtin-ai model: {} and transcription provider: {}",
        model,
    ...
    if let Some(state) = app.try_state::<AppState>() {
        ...
        let (provider, transcription_model) = match transcription_provider.as_str() {
            "whisper" => ("localWhisper", crate::config::DEFAULT_WHISPER_MODEL),
