...
impl WhisperEngine {
    fn get_download_source(model_name: &str) -> Result<(&'static str, &'static str)> {
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        ...
        let models_dir = if let Some(dir) = models_dir {
            ...
            if cfg!(debug_assertions) {
                // Development mode - try local frontend model directories
    ...
    pub async fn download_model(&self, model_name: &str, progress_callback: Option<Box<dyn Fn(u8) + Send>>) -> Result<()> {
        ...
        let (model_url, filename) = Self::get_download_source(model_name)?;
    ...
    pub async fn cancel_download(&self, model_name: &str) -> Result<()> {
        ...
        let (_, filename) = Self::get_download_source(model_name)?;
        let file_path = self.models_dir.join(filename);
        if file_path.exists() {
